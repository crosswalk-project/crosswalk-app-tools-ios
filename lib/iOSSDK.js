...
function iOSSDK(config) {
    ...
    if (typeof(config) != 'undefined' && config != null) {
        ...
        this._sdk = typeof(config.sdk) != 'undefined'?config.sdk:'iphoneos8.1';
        this._sign = typeof(config.sign) != 'undefined'?config.sign:null;
        this._privision = typeof(config.privision) != 'undefined'?config.privision:null;
    }
...
iOSSDK.prototype.getBuildCmd = function(archs, release) {
    ...
    return ['xcodebuild', '-project', xcodeproj_path, archs_param,
            ...
            'USER_HEADER_SEARCH_PATHS=' + Path.join(this._project_path, 'Libs', 'XWalkView.framework', 'Headers')].join(' ');
}
...
iOSSDK.prototype.getXcrunCmd = function() {
    ...
    if (this._privision != null) {
        cmd = cmd.concat(['--embed', '"' + this._privision + '"']);
    }
    return cmd.join(' ');
}
...
iOSSDK.prototype.build = function(archs, release) {
    return Shell.exec(this.getBuildCmd(archs, release)) && Shell.exec(this.getXcrunCmd());
}
